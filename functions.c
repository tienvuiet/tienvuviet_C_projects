...
// Kiem tra ID danh muc co bi trung hay khong
...
// Kiem tra ten danh muc co bi trung hay khong
...
void addCategory() {
    ...
    while (fread(&categories[count], sizeof(Category), 1, file)) {
        count++;
        if (count >= 100) { // Neu vuot qua gioi han 100 danh muc
    ...
    file = fopen("categories.bin", "ab");
    if (file == NULL) {
        printf("Error: Unable to open file for appending.\n");
        return;
    }
    // Nhap so luong danh muc can them
    int numCategories;
...
void deleteCategory() {
    ...
    int maxIdLength = 2;       // Do dai lon nhat cua ID
    int maxNameLength = 4;     // Do dai lon nhat cua ten danh muc
    int i; 
    for (i = 0; i < count; i++) {
    ...
    for (i = 0; i < count; i++) {
        if (categories[i].id == idToDelete) {
            found = 1;
            // Hien thi thong bao xac nhan xoa
            char confirmation;
...
void editCategory() {
    ...
    if (file == NULL) {
        printf("Error: Unable to open file.\n");
        return; // Thoat neu file khong mo duoc
    }
    printf("\n                         **** Category List ****\n");
    Category c;
    int count = 0; // Dem so luong danh muc trong file
    int maxIdLength = 2;       // Chieu dai lon nhat cua ID
    int maxNameLength = 4;     // Chieu dai lon nhat cua ten danh muc
    while (fread(&c, sizeof(Category), 1, file)) {
        count++;
        ...
        if (idLength > maxIdLength) maxIdLength = idLength;
        if ((int)strlen(c.name) > maxNameLength) maxNameLength = strlen(c.name); // Cap nhat do dai ten
    }
    if (count == 0) { // Neu khong co danh muc nao
    ...
    int idToEdit; // ID cua danh muc can chinh sua
    scanf("%d", &idToEdit);
    fflush(stdin);
    ...
    fclose(file); 
    if (!found) { // Neu khong tim thay ID
        printf("        Error: Category with ID %d not found.\n", idToEdit);
        return;
    }
...
void searchCategoryByName() {
    ...
    // Tinh do rong bang truoc neu can
    while (fread(&c, sizeof(Category), 1, file)) {
...
// Ham them san pham vao danh muc
void addProduct() {
    // Mo file danh muc de kiem tra danh muc ton tai
    FILE *categoryFile = fopen("categories.bin", "rb");
    if (categoryFile == NULL) {
        printf("Error: No categories available. Add categories first.\n");
        return; // Thoat neu chua co danh muc
    }

    // Hien thi bang danh muc truoc khi them san pham
    Category category;
    int maxIdLength = 2;       // Chieu dai lon nhat cua ID
    int maxNameLength = 4;     // Chieu dai lon nhat cua ten danh muc

    // Xac dinh do rong cua cac cot trong bang
    ...
    int tableWidth = maxIdLength + maxNameLength + 7; // +7 de bo sung cac ky tu phan cach
    ...
    printf("                              ");
    int i; 
    for ( i = 0; i < tableWidth; i++) printf("="); // Ve duong ke ngang
    printf("\n");
    printf("                              | %-*s | %-*s |\n", maxIdLength, "ID", maxNameLength, "Name"); // Tieu de bang
    printf("                              ");
    for (i = 0; i < tableWidth; i++) printf("="); // Ve duong ke ngang
    printf("\n");

    while (fread(&category, sizeof(Category), 1, categoryFile)) {
        printf("                              | %-*d | %-*s |\n", maxIdLength, category.id, maxNameLength, category.name); // In tung danh muc
    }
    printf("                              ");
    for (i = 0; i < tableWidth; i++) printf("="); // Ve duong ke ngang
    printf("\n");

    fclose(categoryFile); // Dong file danh muc sau khi hien thi

    // Mo file san pham de ghi du lieu
    FILE *productFile = fopen("products.bin", "ab");
    if (productFile == NULL) {
        printf("Error: Unable to open products file.\n");
        return; // Thoat neu file san pham khong mo duoc
    }
    ...
    if (!isCategoryExists(categoryId)) {
        ...
        fclose(productFile); // Dong file san pham neu ID khong hop le
        return;
    }
...
// Ham hien thi san pham theo tung danh muc
void viewProductsByCategory() {
    ...
    int maxQuantityLength = 10; // chieu dai max cua so luong
...
// Ham kiem tra ID danh muc co ton tai hay khong
...
void editProductInformation() {
    ...
    // Mo lai file de ghi cac thay doi
...
void deleteProduct() {
    ...
    // Tinh chieu rong cua cot trong bang
    int i; 
    for (i = 0; i < count; i++) {
        int idLength = snprintf(NULL, 0, "%d", products[i].id);
        if (idLength > maxIdLength) maxIdLength = idLength;
    ...
    int found = 0; // Bien kiem tra san pham co ton tai
    ...
    if (!found) {
        printf("Error: Product with ID %d not found.\n", productId);
        remove("products_temp.bin"); // Xoa file tam neu khong co thay doi
    } else {
        remove("products.bin"); // Xoa file cu
        rename("products_temp.bin", "products.bin"); // Doi ten file tam thanh file chinh
    }
}
...
void filterProductsByPriceRange() {
    FILE *productFile = fopen("products.bin", "rb");
    if (productFile == NULL) {
        // Bao loi neu file san pham khong ton tai
